...
int main (int argc, char *argv[]) {
  ...
  for (int i = 0; i < num_dest; i++) {
    ...
    // printf("check completion of threads \n");
  }
...
pthread_t *launch_threads(int i) {
...
void open_connections(int i) {
  ...
  // for (int i = 0; i < 1; i++) {
    ...
    // /*
    // set client address and port (Dejene)
    // */
    // clntaddr.sin_family=AF_INET;
    // clntaddr.sin_addr.s_addr=INADDR_ANY;
    // clntaddr.sin_port=htons(rand()%65656);

    // if (bind(sock,(struct sockaddr*) &clntaddr, sizeof(struct sockaddr_in))<0)
...
void run_iteration(int it) {
  ...
  for (int j = 0; j < iteration_fanout[it]; j++) {
    ...
    open_connections(iteration_destination[index]);

    // launch threads
    threads = launch_threads(iteration_destination[index]);
    // printf("===\n");
...
void *listen_connection(void *ptr) {
  ...
  while(file_count < dest_file_count[index]) {
    // printf("%d - wait for meta data, left: %d dest_file_count %d\n", index, (dest_file_count[index] - file_count),dest_file_count[index]);
    dest_file_count[index]=1;
    int total = 0;
