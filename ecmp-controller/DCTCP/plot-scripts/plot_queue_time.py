  ...
  for line in open(f).xreadlines():
    ...
    if len(aline)==2 and int(aline[1]) <= (5*1000*args.maxq):
      ...
      if first_time==0:
        ...
        times.append(float(aline[0])-first_time)


  print(len(queue),len(times))
  return queue, times
...
def main():
  ...
  i=0
  # for f in ['dctcp-n3-bw100/qlen_s1-eth1.txt','mdtcp-n3-bw100/qlen_s1-eth1.txt']:
  for f in ['mptcp-n3-bw100/qlen_s1-eth1.txt','tcpecn-n3-bw100/qlen_s1-eth1.txt','tcp-n3-bw100/qlen_s1-eth1.txt']:
  
    queue,times=parse_queue(f)
    ...
    axPlot.plot(times[1:],queue,label=llabel,color=colors[i],linestyle='-',linewidth=1)
