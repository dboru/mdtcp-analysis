...
int main(int argc, char *argv[])
{
    ...
    int sockfd; /* socket */
    int sock_opt = 1;
    struct sockaddr_in serv_addr,my_addr;   /* server address */
    unsigned int fct_us;
    char myIP[16];
    unsigned int myPort;
    ...
    for (i = 0; i < flow_number; i ++)
    {
	   ...
	   //printf("Flow: ID: %u\nSize: %u bytes ToS: %u Rate: %u Mbps\n", flow.id, flow.size, flow.tos, flow.rate);
        printf("Size:%u,Duration(us):%u,serv_ip:%s,serv_port:%u,clnt_ip:%s,clnt_port:%u\n", flow.size,fct_us,server_ip,server_port,myIP,myPort);
    }
